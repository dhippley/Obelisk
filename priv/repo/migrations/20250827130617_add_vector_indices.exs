defmodule Obelisk.Repo.Migrations.AddVectorIndices do
  ...
  def up do
    ...
    execute("CREATE INDEX ON memory_chunks USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)")

    # Optional: also create index on memories table embeddings
    execute("CREATE INDEX ON memories USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)")
  end
